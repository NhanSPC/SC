...
Namespace SC
    ...
    <Serializable()> _
    Public Class WOInfoList
        Inherits Csla.ReadOnlyListBase(Of WOInfoList, WOInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(WO).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As WOInfo In WOInfoList.GetWOInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, WO.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = WOInfoList.GetWOInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of WOInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As WOInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetWOInfo(ByVal pWoId As String) As WOInfo
            Dim Info As WOInfo = WOInfo.EmptyWOInfo(pWoId)
            ContainsCode(pWoId, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pWoId As String) As String
            Return GetWOInfo(pWoId).Description
        End Function

        Public Shared Function GetWOInfoList() As WOInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of WOInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetWOInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pWoId As String, Optional ByRef RetInfo As WOInfo = Nothing) As Boolean

            RetInfo = WOInfo.EmptyWOInfo(pWoId)
            If GetWODic.ContainsKey(pWoId) Then
                RetInfo = GetWODic(pWoId)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("WO"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(ConnectionFactory.GetPhoebusConnectionString)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        'cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim AND TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        '     cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = WOInfo.GetWOInfo(dr)
                                Me.Add(info)
                            End While
                        End Using
                    ...
                    'cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_SC_WO_DEM WHERE DTB.Value.Tri
...
#Region "WO Dictionary"

        Private Shared _woDic As Dictionary(Of String, WOInfo)

        Private Shared Function GetWODic() As Dictionary(Of String, WOInfo)
            If _woDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _woDic = New Dictionary(Of String, WOInfo)

                For Each itm In WOInfoList.GetWOInfoList
                    _woDic.Add(itm.Code, itm)
                Next
            End If
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_SC_WO_DEM  _DTB .Value.Tri

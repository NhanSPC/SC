...
Namespace SC
    ...
    <Serializable()> _
    Public Class WOEInfoList
        Inherits Csla.ReadOnlyListBase(Of WOEInfoList, WOEInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(WOE).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As WOEInfo In WOEInfoList.GetWOEInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, WOE.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = WOEInfoList.GetWOEInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of WOEInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As WOEInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetWOEInfo(ByVal pLineNo As String) As WOEInfo
            Dim Info As WOEInfo = WOEInfo.EmptyWOEInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetWOEInfo(pLineNo).Description
        End Function

        Public Shared Function GetWOEInfoList() As WOEInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of WOEInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetWOEInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As WOEInfo = Nothing) As Boolean

            RetInfo = WOEInfo.EmptyWOEInfo(pLineNo)
            If GetWOEDic.ContainsKey(pLineNo) Then
                RetInfo = GetWOEDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("WOE"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(ConnectionFactory.GetPhoebusConnectionString)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_EVENT_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        'cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_EVENT_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim AND TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        '     cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_EVENT_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        'End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = WOEInfo.GetWOEInfo(dr)
                                Me.Add(info)
                            End While
                        End Using
                    ...
                    'cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_SC_WO_EVENT_DEM WHERE DTB.Value.Tri
...
#Region "WOE Dictionary"

        Private Shared _woeDic As Dictionary(Of String, WOEInfo)

        Private Shared Function GetWOEDic() As Dictionary(Of String, WOEInfo)
            If _woeDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _woeDic = New Dictionary(Of String, WOEInfo)

                For Each itm In WOEInfoList.GetWOEInfoList
                    _woeDic.Add(itm.Code, itm)
                Next
            End If
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_SC_WO_EVENT_DEM  _DTB .Value.Tri

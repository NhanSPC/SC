...
Namespace SC
    ...
    <Serializable()> _
    Public Class WOLInfoList
        Inherits Csla.ReadOnlyListBase(Of WOLInfoList, WOLInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(WOL).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As WOLInfo In WOLInfoList.GetWOLInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, WOL.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = WOLInfoList.GetWOLInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of WOLInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As WOLInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetWOLInfo(ByVal pLineNo As String) As WOLInfo
            Dim Info As WOLInfo = WOLInfo.EmptyWOLInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetWOLInfo(pLineNo).Description
        End Function

        Public Shared Function GetWOLInfoList() As WOLInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of WOLInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetWOLInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As WOLInfo = Nothing) As Boolean

            RetInfo = WOLInfo.EmptyWOLInfo(pLineNo)
            If GetWOLDic.ContainsKey(pLineNo) Then
                RetInfo = GetWOLDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("WOL"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(ConnectionFactory.GetPhoebusConnectionString)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_LABOR_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        'cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_LABOR WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim AND TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        '     cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_LABOR WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = WOLInfo.GetWOLInfo(dr)
                                Me.Add(info)
                            End While
                        End Using
                    ...
                    'cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_SC_WO_LABOR WHERE DTB.Value.Tri
...
#Region "WOL Dictionary"

        Private Shared _wolDic As Dictionary(Of String, WOLInfo)

        Private Shared Function GetWOLDic() As Dictionary(Of String, WOLInfo)
            If _wolDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _wolDic = New Dictionary(Of String, WOLInfo)

                For Each itm In WOLInfoList.GetWOLInfoList
                    _wolDic.Add(itm.Code, itm)
                Next
            End If
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_SC_WO_LABOR  _DTB .Value.Tri

...
Namespace SC
    ...
    <Serializable()> _
    Public Class WOPInfoList
        Inherits Csla.ReadOnlyListBase(Of WOPInfoList, WOPInfo)
...
#Region " Transfer and Report Function "
        ...
        Public Shared Function TransferOut(ByVal Code_From As String, ByVal Code_To As String, ByVal FileName As String) As Integer
            Dim _dt As New DataTable(GetType(WOP).ToString)
            Dim oa As New ObjectAdapter()

            For Each info As WOPInfo In WOPInfoList.GetWOPInfoList
                If info.Code >= Code_From AndAlso info.Code <= Code_To Then
                    oa.Fill(_dt, WOP.GetBO(info.ToString))
                End If
            Next
            Try
        ...
        Public Shared Function GetMyReportDataset() As List(Of DataTable)
            ...
            Dim thelist = WOPInfoList.GetWOPInfoList.ToList()

            Dim shr = New pbs.BO.ObjectShredder(Of WOPInfo)
...
#Region " Business Properties and Methods "
        ...
        Private Shared _list As WOPInfoList
...
#Region " Factory Methods "
        ...
        Public Shared Function GetWOPInfo(ByVal pLineNo As String) As WOPInfo
            Dim Info As WOPInfo = WOPInfo.EmptyWOPInfo(pLineNo)
            ContainsCode(pLineNo, Info)
            Return Info
        End Function

        Public Shared Function GetDescription(ByVal pLineNo As String) As String
            Return GetWOPInfo(pLineNo).Description
        End Function

        Public Shared Function GetWOPInfoList() As WOPInfoList
            If _list Is Nothing Or _DTB <> Context.CurrentBECode Then

                _DTB = Context.CurrentBECode
                _list = DataPortal.Fetch(Of WOPInfoList)(New FilterCriteria())
        ...
        '            ResetCache()
        '        Else
        '            Dim thelist = GetWOPInfoList_Full()
        ...
        Public Shared Function ContainsCode(ByVal pLineNo As String, Optional ByRef RetInfo As WOPInfo = Nothing) As Boolean

            RetInfo = WOPInfo.EmptyWOPInfo(pLineNo)
            If GetWOPDic.ContainsKey(pLineNo) Then
                RetInfo = GetWOPDic(pLineNo)
                Return True
            End If
        ...
        '        Return True
        '    Else
        '        e.Description = String.Format(ResStr(Msg.NOSUCHITEM), ResStr("WOP"), value)
        '        Return False
        '    End If
        'End Function
            ...
            SyncLock _lockObj
                ...
                Using cn = New SqlClient.SqlConnection(ConnectionFactory.GetPhoebusConnectionString)
                    ...
                    Using cm = cn.CreateCommand()
                        ...
                        cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_PARTS_<%= _DTB %></SqlText>.Value.Trim

                        ' If criteria._timeStamp IsNot Nothing AndAlso criteria._timeStamp.Length > 0 Then
                        'cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_PARTS_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim AND TIME_STAMP > @CurrentTimeStamp.Value.Trim
                        '     cm.Parameters.AddWithValue("@CurrentTimeStamp", criteria._timeStamp)
                        ' Else
                        '     cm.CommandText = <SqlText>SELECT * FROM pbs_SC_WO_PARTS_DEM WHERE DTB='<%= _DTB %>'</SqlText>.Value.Trim
                        ' End If

                        Using dr As New SafeDataReader(cm.ExecuteReader)
                            While dr.Read
                                Dim info = WOPInfo.GetWOPInfo(dr)
                                Me.Add(info)
                            End While
                        End Using
                    ...
                    'cm.CommandText = SELECT max(TIME_STAMP) FROM pbs_SC_WO_PARTS_DEM WHERE DTB.Value.Tri
...
#Region "WOP Dictionary"

        Private Shared _wopDic As Dictionary(Of String, WOPInfo)

        Private Shared Function GetWOPDic() As Dictionary(Of String, WOPInfo)
            If _wopDic Is Nothing OrElse _DTB <> Context.CurrentBECode Then
                _wopDic = New Dictionary(Of String, WOPInfo)

                For Each itm In WOPInfoList.GetWOPInfoList
                    _wopDic.Add(itm.Code, itm)
                Next
            End If
...
#Region "TimeStamp"
        ...
        '    Dim script = SELECT count(*) FROM pbs_SC_WO_PARTS_DEM  _DTB .Value.Tri
